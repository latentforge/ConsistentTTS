"""Configuration class for Higgs-Audio v3."""

from transformers.configuration_utils import PreTrainedConfig
from transformers.models.qwen3.configuration_qwen3 import Qwen3Config


class HiggsAudioV3AudioEncoderConfig(PreTrainedConfig):
    r"""
    This is the configuration class to store the configuration of the fused multi-codebook audio embedding/head of
    a [`HiggsAudioV3ForConditionalGeneration`]. It does not describe a standalone encoder module: Higgs-Audio v3
    encodes and decodes waveforms with the transformers-native [`HiggsAudioV2TokenizerModel`], loaded separately
    from `audio_tokenizer_id` on the parent [`HiggsAudioV3Config`].
    ...
    model_type = "higgs_audio_v3_encoder"
...
class HiggsAudioV3Config(PreTrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`HiggsAudioV3ForConditionalGeneration`]. It is
    used to instantiate a Higgs-Audio v3 model according to the specified arguments, defining a [`Qwen3Model`] text
    ...
    Args:
        ...
        audio_encoder_config ([`HiggsAudioV3AudioEncoderConfig`], *optional*):
    ...
    ```python
    >>> from voicestudio.models.higgs_audio_v3 import HiggsAudioV3Config, HiggsAudioV3ForConditionalGeneration

    >>> configuration = HiggsAudioV3Config()

    >>> model = HiggsAudioV3ForConditionalGeneration(configuration)

    >>> configuration = model.config
    ```"""

    model_type = "higgs_audio_v3"
    sub_configs = {"text_config": Qwen3Config, "audio_encoder_config": HiggsAudioV3AudioEncoderConfig}
    keys_to_ignore_at_inference = ["past_key_values"]

    def __init__(
        self,
        text_config: Qwen3Config | dict | None = None,
        audio_encoder_config: HiggsAudioV3AudioEncoderConfig | dict | None = None,
        ...
        **kwargs,
        ...
        if isinstance(audio_encoder_config, dict):
            audio_encoder_config = HiggsAudioV3AudioEncoderConfig(**audio_encoder_config)
        elif audio_encoder_config is None:
            audio_encoder_config = HiggsAudioV3AudioEncoderConfig()
...
__all__ = ["HiggsAudioV3AudioEncoderConfig", "HiggsAudioV3Config"]
