...
from .configuration_higgs_audio_v3 import HiggsAudioV3Config
...
@auto_docstring
class HiggsAudioV3PreTrainedModel(HiggsAudioV2PreTrainedModel):
...
@auto_docstring
class HiggsAudioV3Model(HiggsAudioV3PreTrainedModel):
...
@auto_docstring(
    custom_intro="""
    The Higgs Audio v3 model, a Qwen3-backbone auto-regressive transformer paired with a fused multi-codebook
    audio embedding and output head.
    """
)
class HiggsAudioV3ForConditionalGeneration(HiggsAudioV3PreTrainedModel):
    base_model_prefix = "model"
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        audio_labels (`torch.LongTensor` of shape `(batch_size, num_audio_frames, num_codebooks)`, *optional*):
            ...
            Can be obtained using `output_labels=True` when calling [`HiggsAudioV3Processor`].
        ...
        ```python
        >>> from voicestudio.models.higgs_audio_v3 import HiggsAudioV3ForConditionalGeneration, HiggsAudioV3Processor
        >>> model_id = "bosonai/higgs-tts-3-4b"
        >>> processor = HiggsAudioV3Processor.from_pretrained(model_id)
        >>> model = HiggsAudioV3ForConditionalGeneration.from_pretrained(model_id)
        ...
        **kwargs,
        ...
        Args:
            ...
            audio_input_ids (`torch.LongTensor` of shape `(1, num_audio_frames, num_codebooks)`, *optional*):
                Reference-audio codes to condition generation on, as built by [`HiggsAudioV3Processor`].
        ...
        Returns:
            ...
            codes, ready for [`HiggsAudioV3Processor.decode`].
        """
        if input_ids.shape[0] != 1:
            raise ValueError("HiggsAudioV3ForConditionalGeneration.generate only supports batch_size=1.")
...
__all__ = ["HiggsAudioV3ForConditionalGeneration", "HiggsAudioV3Model", "HiggsAudioV3PreTrainedModel"]
