# coding=utf-8
# Copyright 2024 Yoach Lacombe, Sanchit Gandhi, the HuggingFace Inc. team and the LatentForge team.
# All rights reserved.
#
...
from transformers.generation import GenerationMixin, LogitsProcessor
...
class ParlerTTSLogitsProcessor(LogitsProcessor):
    r"""
    Ensures that each codebook stops generating (emits the eos token) only once every codebook that precedes it
    in delay order has already stopped, matching the model's delay-pattern decoding scheme.

    Args:
        eos_token_id (`Union[int, List[int], torch.Tensor]`):
            The id(s) of the *end-of-sequence* token.
        num_codebooks (`int`):
            Number of codebooks generated by the model.
        batch_size (`int`):
            Number of samples generated in parallel.
        device (`str`, *optional*, defaults to `"cpu"`):
            Device on which to allocate this processor's bookkeeping tensors.
    """
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        self.first_codebooks_unfinished = torch.where(
            ...
            self.first_codebooks_unfinished + 1,
            self.first_codebooks_unfinished,
        )
...
PARLER_TTS_START_DOCSTRING = r"""

    Parler-TTS was proposed in [Natural language guidance of high-fidelity text-to-speech with synthetic
    annotations](https://arxiv.org/abs/2402.01912) by Dan Lyth and Simon King. It is an encoder-decoder transformer,
    derived from the Musicgen architecture, that conditions audio-codec token generation on a text description of
    the desired voice and a text prompt of the words to speak.
...
PARLER_TTS_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_input_ids (`torch.LongTensor` of shape `(batch_size * num_codebooks, target_sequence_length)`, *optional*):
            ...
            such as with the [`DacModel`]. See [`DacModel.encode`] for details.
            ...
            you obtain audio codes from an audio encoding model, such as [`DacModel`], ensure that the number of
...
PARLER_TTS_DECODER_INPUTS_DOCSTRING = r"""
    Args:
...
class ParlerTTSDecoder(ParlerTTSPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(PARLER_TTS_DECODER_INPUTS_DOCSTRING)
    def forward(
        self,
...
class ParlerTTSForConditionalGeneration(PreTrainedModel, GenerationMixin):
    ...
    @add_start_docstrings_to_model_forward(PARLER_TTS_INPUTS_DOCSTRING)
