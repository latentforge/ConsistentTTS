"""Processor class for Higgs-Audio v3."""
...
class HiggsAudioV3Processor(ProcessorMixin):
    r"""
    Constructs a Higgs-Audio v3 processor which wraps a [`DacFeatureExtractor`], an [`AutoTokenizer`], and a
    [`HiggsAudioV2TokenizerModel`] into a single processor. See [`~HiggsAudioV3Processor.__call__`] and
    [`~HiggsAudioV3Processor.decode`] for more information.
    ...
    def __call__(
        ...
        **kwargs: Unpack[HiggsAudioV3ProcessorKwargs],
        ...
        Returns:
            [`BatchFeature`]: Ready to be passed to [`HiggsAudioV3ForConditionalGeneration`], with `input_ids`,
            `attention_mask`, and, when `reference_audio` is provided, `audio_input_ids`/`audio_input_ids_mask`.
        """
        output_kwargs = self._merge_kwargs(
            HiggsAudioV3ProcessorKwargs,
    ...
    def decode(self, audio_input_ids: torch.LongTensor) -> torch.Tensor:
        ...
        Args:
            audio_input_ids (`torch.LongTensor` of shape `(1, sequence_length, num_codebooks)`):
                Delayed audio codes as produced by [`HiggsAudioV3ForConditionalGeneration.generate`].
